      ...
      <div className="py-8 md:py-10 lg:py-12 space-y-12 no-print">
        <header className="flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-5xl font-bold text-white font-display tracking-tight text-glow-cyan uppercase">Student Schedule</h1>
              <p className="text-muted-foreground text-lg font-medium italic">Academic success timeline via PyroX Control.</p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <Button onClick={printSingle} disabled={!selectedStudentId} className="h-16 px-6 bg-accent text-background font-black shadow-neon-cyan flex-1 transition-transform active:scale-95 uppercase">
                <Printer className="mr-2 h-5 w-5" /> Print My
              </Button>
              <Button onClick={printAll} variant="outline" className="h-16 px-6 border-accent text-accent font-black hover:bg-accent/10 flex-1 transition-transform active:scale-95 uppercase">
        ...
        {selectedStudentId ? (
          <div className="space-y-4">
            <div className="flex items-center justify-between px-4">
              <h3 className="text-xl font-bold text-accent uppercase tracking-widest">Active Timeline: {selectedStudent?.name}</h3>
              <span className="text-xs font-mono text-muted-foreground uppercase">Sync Status: Optimized</span>
            </div>
          ...
          <div className="glass-metallic h-[600px] flex flex-col items-center justify-center rounded-3xl border-dashed border-2 border-accent/30 space-y-6">
            ...
            <h2 className="text-3xl font-bold text-muted-foreground uppercase tracking-widest">Identify a learner to visualize trajectory</h2>
          </div>
        )}
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="glass-metallic neon-border-cyan border-2 sm:max-w-[500px] p-8" aria-describedby="student-dialog-desc">
            <DialogHeader>
              <DialogTitle className="text-3xl font-bold text-white mb-2 uppercase tracking-tighter">Schedule Session</DialogTitle>
              <DialogDescription id="student-dialog-desc" className="text-accent font-mono uppercase font-bold">{activeCell?.day} // {activeCell?.time}</DialogDescription>
            </DialogHeader>
            <div className="space-y-6 py-4">
              <div className="space-y-2">
                <Label className="text-lg font-bold text-white uppercase tracking-widest text-xs">Module ID</Label>
                {useCustomSubject ? (
                  <div className="flex gap-2">
                    <Input value={subject} onChange={(e) => setSubject(e.target.value)} className="h-14 bg-secondary/50 border-accent/30 text-white font-bold" placeholder="Identify custom module..." autoFocus />
              ...
              <div className="space-y-2">
                <Label className="text-lg font-bold text-white uppercase tracking-widest text-xs">Session Protocol</Label>
                <Input value={notes} onChange={(e) => setNotes(e.target.value)} className="h-14 bg-secondary/50 border-accent/30 text-white font-bold" placeholder="E.g. Advanced Calculus Focus" />
              </div>
            </div>
            <DialogFooter>
              <Button onClick={handleSave} className="w-full h-16 text-xl font-bold bg-accent text-background hover:bg-accent/90 shadow-neon-cyan uppercase tracking-widest">
