...
function TutorScheduleReport({ tutorId, tutorName, mode }: { tutorId: Id<"tutors">, tutorName: string, mode: string }) {
  const schedule = useQuery(api.pirox.getSchedule, { ownerId: tutorId }) ?? [];
  return (
    <div className="mb-20 page-break-after border-b border-gray-200 pb-10">
      <div className="flex justify-between items-end border-b-2 border-black pb-4 mb-6">
        <div>
          <h2 className="text-3xl font-bold text-black uppercase tracking-tighter">Expert Deployment Dispatch</h2>
          <p className="text-sm font-mono text-gray-600">Leader ID: {tutorId}</p>
        </div>
        <div className="text-right">
          <p className="text-2xl font-bold text-black">{tutorName}</p>
          <p className="text-md font-bold uppercase text-gray-500">{mode} Instruction</p>
        </div>
      </div>
...
export function TutorSchedulePage() {
  ...
  const sendEmail = () => {
    ...
    const subjectLine = `PyroX iTutor: Weekly Schedule for ${selectedTutor.name}`;
    ...
    days.forEach(day => {
      ...
      if (daySlots.length > 0) {
        ...
        daySlots.forEach(s => {
          body += `● ${s.timeSlot}: ${s.subject}${s.notes ? ` — Note: ${s.notes}` : ""}\n`;
        });
        body += `\n`;
      }
    });
    body += `\nSent via PyroX-iTutor Admin Control.`;
  ...
  const emailAllSchedules = () => {
    ...
    const emailTo = "nkwembeyazw@gmail.com";
    const subjectLine = `PyroX: Roster Dispatch ${format(new Date(), "MM/dd")}`;
    let body = `URGENT: AGGREGATE EXPERT DISPATCH SUMMARY\n\n`;
    tutors.forEach(t => {
      body += `▶ ${t.name} (${t.mode})\n`;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="py-8 md:py-10 lg:py-12 space-y-12 no-print">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <Button onClick={sendEmail} disabled={!selectedTutorId} className="h-16 px-6 bg-white text-primary font-bold shadow-xl flex-1 transition-transform active:scale-95">
                <Mail className="mr-2 h-5 w-5" /> Email My
              </Button>
              <Button onClick={emailAllSchedules} className="h-16 px-6 bg-primary text-white font-bold shadow-neon-red flex-1 transition-transform active:scale-95">
        ...
        <div className="flex justify-end gap-4 no-print">
           <Button onClick={printSingle} disabled={!selectedTutorId} variant="outline" className="h-14 px-8 border-primary text-primary hover:bg-primary/10 transition-transform active:scale-95">
              <Printer className="mr-2 h-6 w-6" /> Print My Roster
           </Button>
           <Button onClick={printAll} className="h-14 px-8 bg-primary text-white font-bold shadow-neon-red transition-transform active:scale-95">
              <Files className="mr-2 h-6 w-6" /> Print All Tutors
           </Button>
        </div>
        {selectedTutorId ? (
          ...
          <DialogContent className="glass-metallic neon-border-red border-2 sm:max-w-[500px] p-8" aria-describedby="tutor-dialog-desc">
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="space-y-2">
                ...
                <Input value={notes} onChange={(e) => setNotes(e.target.value)} className="h-14 bg-secondary/50 border-primary/30 text-white" placeholder="Focus on specifics" />
              </div>
            </div>
            <DialogFooter>
