...
export function TutorSchedulePage() {
  ...
  const emailAllSchedules = () => {
    const emailTo = "nkwembeyazw@gmail.com";
    const subjectLine = `PyroX iTutor: AGGREGATE ROSTER DISPATCH - ${new Date().toLocaleDateString()}`;
    let body = `URGENT: AGGREGATE TUTOR ROSTER\n\n`;
    tutors.forEach(t => {
      body += `>>> EXPERT: ${t.name} (${t.mode})\n`;
      body += `Disciplines: ${t.subjects.join(", ")}\n`;
      body += `Contact: ${t.contact}\n`;
      body += `----------------------------------------\n\n`;
    });
    body += `\nRefer to the PyroX Dashboard for granular time-slot details.\n\nGenerated by PyroX Admin Control.`;
    window.location.href = `mailto:${emailTo}?subject=${encodeURIComponent(subjectLine)}&body=${encodeURIComponent(body)}`;
  };
