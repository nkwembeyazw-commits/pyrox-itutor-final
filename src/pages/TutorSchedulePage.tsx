...
export function TutorSchedulePage() {
  ...
  const assignedStudents = students.filter(s => selectedTutor?.studentIds.includes(s._id));
  ...
  const handleSave = async () => {
    ...
    try {
      await upsertSlot({
        ...
        subject,
        notes,
        studentId: selectedStudentId === "none" ? undefined : selectedStudentId,
      ...
      <div className="py-8 md:py-10 lg:py-12 space-y-8 no-print">
        ...
        <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-3xl md:text-5xl font-bold text-white font-display tracking-tight text-glow-red uppercase leading-tight">Tutor Dispatch</h1>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full xl:w-auto">
            ...
            <div className="flex gap-2 shrink-0">
              ...
              <Button onClick={emailAllSchedules} className="h-14 px-4 bg-primary text-white font-black shadow-neon-red flex-1 transition-transform active:scale-95 uppercase text-xs">
                <Send className="mr-2 h-4 w-4" /> Email All
              </Button>
            </div>
          </div>
        </header>
        <div className="flex flex-wrap justify-end gap-2 no-print">
           <Button onClick={() => window.print()} disabled={!selectedTutorId} variant="outline" className="h-12 px-6 border-primary text-primary font-black hover:bg-primary/10 transition-transform active:scale-95 uppercase text-xs">
              <Printer className="mr-2 h-4 w-4" /> Print My
           </Button>
           <Button onClick={() => setIsPrintingAll(true)} className="h-12 px-6 bg-primary text-white font-black shadow-neon-red transition-transform active:scale-95 uppercase text-xs">
        ...
        {selectedTutorId ? (
          <div className="space-y-4">
             <div className="flex items-center justify-between px-2">
              ...
              <span className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest hidden sm:inline">Active Dispatch</span>
            </div>
          ...
          <div className="glass-metallic h-[400px] md:h-[600px] flex flex-col items-center justify-center rounded-3xl border-dashed border-2 border-primary/30 space-y-6">
            <BookOpen className="h-24 w-24 md:h-32 md:w-32 text-primary/20 animate-pulse" />
            <h2 className="text-xl md:text-3xl font-bold text-muted-foreground text-center uppercase tracking-widest px-4">Enlist an expert to coordinate their timeline</h2>
          </div>
        )}
