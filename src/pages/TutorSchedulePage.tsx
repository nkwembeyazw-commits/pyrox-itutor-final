...
export function TutorSchedulePage() {
  ...
  const emailAllSchedules = () => {
    ...
    const emailTo = "nkwembeyazw@gmail.com";
    const subjectLine = `PyroX iTutor: AGGREGATE ROSTER DISPATCH - ${format(new Date(), "PP")}`;
    let body = `URGENT: AGGREGATE EXPERT DISPATCH SUMMARY\n========================================\n\n`;
    tutors.forEach(t => {
      body += `▶ EXPERT: ${t.name}\n`;
      body += `  Mode: ${t.mode}\n`;
      body += `  Disciplines: ${t.subjects.join(", ")}\n`;
      body += `  Contact: ${t.contact}\n`;
      body += `----------------------------------------\n\n`;
    });
    body += `\nRefer to the PyroX Dashboard for granular session timestamps.\n\nGenerated by PyroX Automated Systems.`;
    // Safety check for mailto length limits (browser specific, but 2000 is a safe threshold)
    if (body.length > 1800) {
      toast.warning("Roster summary is large; content may be truncated in some email clients.");
    }
    window.location.href = `mailto:${emailTo}?subject=${encodeURIComponent(subjectLine)}&body=${encodeURIComponent(body)}`;
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="py-8 md:py-10 lg:py-12 space-y-12 no-print">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <Button onClick={sendEmail} disabled={!selectedTutorId} className="h-16 px-6 bg-white text-primary font-bold shadow-xl flex-1">
                <Mail className="mr-2 h-5 w-5" /> Email My
              </Button>
              <Button onClick={emailAllSchedules} className="h-16 px-6 bg-primary text-white font-bold shadow-neon-red flex-1">
        ...
        <div className="flex justify-end gap-4 no-print">
           <Button onClick={printSingle} disabled={!selectedTutorId} variant="outline" className="h-14 px-8 border-primary text-primary hover:bg-primary/10">
              <Printer className="mr-2 h-6 w-6" /> Print My Roster
           </Button>
           <Button onClick={printAll} className="h-14 px-8 bg-primary text-white font-bold shadow-neon-red">
      ...
      <div className="hidden print:block print-section p-10 bg-white text-black">
        {selectedTutorId && !document.body.classList.contains('print-all-mode') ? (
          <TutorScheduleReport
